    ...
    try {
      ...
      if (mode === 'url' && this.url) {
        ...
        for (const item of listings) {
          ...
          await this.events.publish(this.normalize(item, { stableEventId: true, mockVelocity: false }));
        }
      } else if (mode === 'api') {
        ...
        for (const item of this.mockListings()) {
          await this.events.publish(this.normalize(item, { stableEventId: false, mockVelocity: true }));
        }
      }
    } catch (err) {
